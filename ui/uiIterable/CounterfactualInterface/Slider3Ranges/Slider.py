...
# this class is the ground to the ranges, with that it is possible to know where are the bounds,
...
class Slider(QLabel):
    ...
    def getValueAtXPosition(self, x):
        ...
        value = self.minValue+((self.maxValue-self.minValue)*((x-minX)/(maxX-minX)))
    ...
    def getPositionFromValue(self, minX, maxX, value):
        ...
        posX = minX+((maxX-minX)*((value-self.minValue)/(self.maxValue-self.minValue)))
    ...
    def getValueFromPos(self, pos):
        ...
        if self.__decimalPlaces == 0:
            value = int(round(self.minValue+pos*(self.maxValue-self.minValue)/(maxX-minX), self.__decimalPlaces))
        else:
            value = round(self.minValue+pos*(self.maxValue-self.minValue)/(maxX-minX), self.__decimalPlaces)
    ...
    def setValue(self, value):
        ...
        if (value >= self.minValue and value <= self.maxValue):
            ...
            valueNorm = (self.value-self.minValue)/(self.maxValue-self.minValue)
