...
from ui.engine.CounterfactualEngineEnums import CounterfactualEngineEnums

class CounterfactualEngine():
    @staticmethod
    def trainRandomForestClassifier(xTrain, yTrain):
        """ Trains and return a trained random forest. """
        assert xTrain is not None
        assert yTrain is not None
        assert len(xTrain) != 0
        assert len(yTrain) != 0
        randomForestClassifier = RandomForestClassifier(
            max_leaf_nodes=CounterfactualEngineEnums.RandomForestClassifierParameters.MAX_LEAF_NODES.value,
    ...
    def trainIsolationForest(xTrain):
        """ Trains and return a trained isolation forest. """
        assert xTrain is not None
        assert len(xTrain) != 0
        isolationForest = IsolationForest(
            random_state=CounterfactualEngineEnums.IsolationForestParameters.RANDOM_STATE.value,
    ...
    def randomForestClassifierPredict(randomForestClassifier, xValue):
        """ Returns the prediction given a model and a point. """
