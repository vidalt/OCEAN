...
def test_mip_explain(
    ...
    try:
        model.explain(x, y=0, norm=1, num_workers=num_workers, random_seed=seed)
        assert model.Status == gp.GRB.OPTIMAL
        model.cleanup()
        model.explain(x, y=0, norm=1, return_callback=True, num_workers=num_workers, random_seed=seed)
...
def test_cp_explain(
    ...
    try:
        _ = model.explain(x, y=0, norm=1, return_callback=False,num_workers=num_workers, random_seed=seed)
        assert model.callback is None or len(model.callback.sollist) == 0
        model.cleanup()
        _ = model.explain(x, y=0, norm=1, return_callback=True, num_workers=num_workers, random_seed=seed)
        assert model.callback is None or len(model.callback.sollist) != 0
    except gp.GurobiError as e:
        pytest.skip(f"Skipping test due to {e}")
