...
def min_max_scaling_with_type(df):
    # copy the dataframe
    df_norm = df.copy()
    columnFeatures = {column:getFeatureType(df[column][0]) for column in df.columns}
...
class DatasetReader:
    def __init__(self, filename, rangeFeasibilityForDiscreteFeatures = False):
        ...
        columnFeatures = {column:getFeatureType(self.data[column][0]) for column in self.data.columns if column != 'Class'}
        self.data = self.data.drop(0)
        columnActionnability = {column:getFeatureActionnability(self.data[column][1]) for column in self.data.columns if column != 'Class'}
        ...
        for column in self.data.columns:
            if len(self.data[column].unique())==1:
                print("Drop column", column, "because it contains a unique value")
                ...
                if len(values) != 2:
                    ...
                    for val in values:
                        if val not in  [0,1,'0','1']:
                            ok=False
                        if not ok:
                            print("error, more than two values in ", column, "which is indicated to be binary, treated as categorical")
                            columnFeatures[column] = FeatureType.Categorical
                if values[0] in ['0','1'] and values[1] in ['0','1']:
                    continue
                assert values[1] != '0'
                self.data[column] = self.data[column].str.replace(values[0],'0')
                self.data[column] = self.data[column].str.replace(values[1],'1')

        # One hot encoding
        oneHotEncoding = dict()
        categoricalColumns = [str(column) for column in self.data.columns if column != 'Class' and columnFeatures[column] == FeatureType.Categorical]
        ...
        for categoricalColumn in categoricalColumns:
            ...
            for column in self.data.columns:
                ...
                c += 1
        self.data = self.data[[col for col in self.data if col != 'Class'] + ['Class']]


        # Reconstruct features type and one hot encoding
        self.featuresType = [columnFeatures[column] for column in self.data.columns if column != 'Class']
        self.featuresActionnability = [columnActionnability[column] for column in self.data.columns if column != 'Class']
            ...
            if column != 'Class':
                if self.featuresType[c] == FeatureType.Categorical:
                    ...
                    if rangeFeasibilityForDiscreteFeatures:
                        self.featuresPossibleValues.append([i/self.upperBoundsList[c] for i in range(0, int(self.upperBoundsList[c]+1))])
                    else:
                        self.featuresPossibleValues.append(self.data[column].unique())
        ...
        self.XwithGoodPoint = self.data.loc[self.data['Class'] == 1,self.data.columns != 'Class']
    ...
    def readRandomForestFromPickleAndApplyMinMaxScaling(self, serializedObject) -> RandomForestClassifier:
        ...
        clfRead : RandomForestClassifier = pickle.load(open(serializedObject, "rb"))
...
def min_max_scaling_with_type_oldDatasetsWithoutActionnability(df):
    # copy the dataframe
    df_norm = df.copy()
    columnFeatures = {column:getFeatureType_oldDatasetsWithoutActionnability(df[column][0]) for column in df.columns}
    df_norm = df_norm.drop(0)
    # Convert types
    for column in df_norm.columns:
        if columnFeatures[column] in [FeatureType.Discrete, FeatureType.Numeric]:
...
class DatasetReader_oldDatasetsWithoutActionnability:
    def __init__(self, filename):
        ...
        # Get the datatypes and remove the corresponding row
        self.featuresType = [getFeatureType_oldDatasetsWithoutActionnability(data[column][0]) for column in data.columns if column != 'Class']
