...
class CuiRandomForestCounterFactualMilp(ClassifierCounterFactualMilp):
    ...
    def buildLevels(self):
        ...
        self.indexOfCellOnTheLeftOfTreeVertexThres = {
            t: dict() for t in range(self.completeForest.n_estimators)}
        self.cellLowerBound = dict()
        self.cellUpperBound = dict()
        for f in range(self.nFeatures):
            self.numberOfCellsOfFeature[f] = len(self.planes[f]) + 1
            self.cellLowerBound[f] = {
                cell: -eps for cell in range(self.numberOfCellsOfFeature[f])}
            self.cellUpperBound[f] = {
                cell: 1.0 for cell in range(self.numberOfCellsOfFeature[f])}
            countCell = 0
    ...
    def buildCellVariablesAndConstraints(self):
        ...
        for f in range(self.nFeatures):
            ...
            for c in range(self.numberOfCellsOfFeature[f]):
                self.cellVar[f][c] = self.model.addVar(
                    vtype=GRB.BINARY, name="cellVar_f" + str(f) + "_c" + str(c))
                sumOfCell += self.cellVar[f][c]
            self.oneCellChosenConstr[f] = self.model.addConstr(
                sumOfCell == 1, "oneCellChosenConstr_f"+str(f))
        ...
        for t in range(self.completeForest.n_estimators):
            ...
            for v in range(tree.node_count):
                if tree.children_left[v] == tree.children_right[v]:
                    self.leafVar[t][v] = self.model.addVar(
                        vtype=GRB.BINARY, name="leafVar_t" + str(t) + "_v" + str(v))
                    sumOfLeaf += self.leafVar[t][v]
            self.oneLeafChosenConstr[t] = self.model.addConstr(
                sumOfLeaf == 1, "oneLeafChosenConstr_t" + str(t))
    ...
    def buildConsistencyConstraints(self):
        ...
        for t in range(self.completeForest.n_estimators):
            ...
            # self.parent[t] = dict()
            self.depth[t] = dict()
            self.nodeLeftMostCellAlongFeature_tvf[t][0] = {
                f: 0 for f in range(self.nFeatures)}
            self.nodeRightMostCellAlongFeature_tvf[t][0] = {
                f: (self.numberOfCellsOfFeature[f]-1) for f in range(self.nFeatures)}
            self.nodeAncestorsAlongFeature_tvf[t][0] = {
                f: [] for f in range(self.nFeatures)}
            self.depth[t][0] = 0
            ...
            while len(stack) > 0:
                ...
                if cr != cl:
                    ...
                    self.nodeRightMostCellAlongFeature_tvf[t][cl][tree.feature[v]
                                                                  ] = self.indexOfCellOnTheLeftOfTreeVertexThres[t][v]
                    self.nodeAncestorsAlongFeature_tvf[t][cl] = {f: self.nodeAncestorsAlongFeature_tvf[t][v][f].copy(
                    ) for f in self.nodeAncestorsAlongFeature_tvf[t][v]}
                    self.nodeAncestorsAlongFeature_tvf[t][cl][tree.feature[v]].append(
                        cl)
                    stack.append(cl)
                    # self.parent[cr] = v
                    ...
                    self.nodeLeftMostCellAlongFeature_tvf[t][cr][tree.feature[v]
                                                                 ] = self.indexOfCellOnTheLeftOfTreeVertexThres[t][v] + 1
                    self.nodeAncestorsAlongFeature_tvf[t][cr] = {f: self.nodeAncestorsAlongFeature_tvf[t][v][f].copy(
                    ) for f in self.nodeAncestorsAlongFeature_tvf[t][v]}
                    self.nodeAncestorsAlongFeature_tvf[t][cr][tree.feature[v]].append(
                        cr)
                    stack.append(cr)
                ...
                for f in range(self.nFeatures):
                    for child in self.nodeAncestorsAlongFeature_tvf[t][leaf][f]:
                        for cell in range(self.nodeLeftMostCellAlongFeature_tvf[t][child][f], self.nodeRightMostCellAlongFeature_tvf[t][child][f] + 1):
                            upperBound += 1 / \
                                self.depth[t][leaf] * self.cellVar[f][cell]
                self.model.addConstr(
                    self.leafVar[t][leaf] <= upperBound, "consistency_tree" + str(t) + "_leaf" + str(leaf))
                ...
                if self.featuresType[f] == FeatureType.Numeric:
                    ...
                    for cell in self.cellVar[f]:
                        ...
                        if cellValues:
                            ...
                            self.model.addConstr(
                                self.cellVar[f][cell] == 0, "empty_cell_f" + str(f) + "_c" + str(cell))
                else:
            ...
            for f in self.cellVar:
                if self.featuresType[f] == FeatureType.Numeric:
                    for cell in self.cellVar[f]:
                        if self.cellUpperBound[f][cell] < self.x0[0][f]:
                            self.nearestInCell[f][cell] = self.cellUpperBound[f][cell] - eps
                            obj += (self.x0[0][f] - self.nearestInCell[f]
                                    [cell]) * self.cellVar[f][cell]
                        elif self.cellLowerBound[f][cell] > self.x0[0][f]:
                            self.nearestInCell[f][cell] = self.cellLowerBound[f][cell] + eps
                            obj += (self.nearestInCell[f][cell]
                                    - self.x0[0][f]) * self.cellVar[f][cell]
                        else:
                            self.nearestInCell[f][cell] = self.x0[0][f]
            ...
            for f in self.cellVar:
                if self.featuresType[f] == FeatureType.Numeric:
                    for cell in self.cellVar[f]:
                        if self.cellUpperBound[f][cell] < self.x0[0][f]:
                            self.nearestInCell[f][cell] = self.cellUpperBound[f][cell] - eps
                            obj += (self.x0[0][f] - self.nearestInCell[f][cell]) * (
                                self.x0[0][f] - self.nearestInCell[f][cell]) * self.cellVar[f][cell]
                        elif self.cellLowerBound[f][cell] > self.x0[0][f]:
                            self.nearestInCell[f][cell] = self.cellLowerBound[f][cell] + eps
                            obj += (self.nearestInCell[f][cell] - self.x0[0][f]) * (
                                self.nearestInCell[f][cell] - self.x0[0][f]) * self.cellVar[f][cell]
                        else:
                            self.nearestInCell[f][cell] = self.x0[0][f]
                elif self.featuresType[f] == FeatureType.Discrete or self.featuresType[f] == FeatureType.Binary:
                    for cell in self.cellVar[f]:
    ...
    def addMajorityVoteConstraint(self):
        ...
        for t in self.completeForest.randomForestEstimatorsIndices:
            for leaf in self.leafVar[t]:
                leaf_val = self.completeForest.estimators_[
                    t].tree_.value[leaf][0]
                tot = sum(leaf_val)
                for output in range(len(leaf_val)):
                    if output == self.outputDesired:
                        for cl in majorityVoteExpr:
                            majorityVoteExpr[cl] += self.leafVar[t][leaf] * (
                                leaf_val[output])/(tot * self.completeForest.n_estimators)
                    else:
                        majorityVoteExpr[output] -= self.leafVar[t][leaf] * \
                            (leaf_val[output]) / \
    ...
    def addIsolationForestPlausibilityConstraint(self):
        ...
        for t in self.completeForest.isolationForestEstimatorsIndices:
            ...
            for leaf in self.leafVar[t]:
                leafDepth = self.depth[t][leaf] + \
                    _average_path_length([tree.tree_.n_node_samples[leaf]])[0]
            ...
            if self.featuresActionnability[f] == FeatureActionnability.Increasing:
                ...
                for cell in range(self.numberOfCellsOfFeature[f]):
                    if self.cellLowerBound[f][cell] < self.x0[0][f] and self.cellUpperBound[f][cell] >= self.x0[0][f]:
                        self.model.addConstr(
                            self.cellVar[f][cell] == 1, "actionnability_f"+str(f))
