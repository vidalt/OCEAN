...
class ClassifierCounterFactualMilp:
    def __init__(
        ...
        if not featuresType:
            self.featuresType = [FeatureType.Numeric for f in range(self.nFeatures)]
        else:
        ...
        self.discreteFeatures = [i for i in range(self.nFeatures) if self.featuresType[i] == FeatureType.Discrete]
        self.categoricalNonOneHotFeatures = [i for i in range(self.nFeatures) if self.featuresType[i] == FeatureType.CategoricalNonOneHot]
        self.discreteAndCategoricalNonOneHotFeatures = [i for i in range(self.nFeatures) if self.featuresType[i] in [FeatureType.CategoricalNonOneHot, FeatureType.Discrete]]
        for f in self.discreteAndCategoricalNonOneHotFeatures:
            ...
            self.featuresPossibleValues[f] = sorted(self.featuresPossibleValues[f])
            for v in range(len(self.featuresPossibleValues[f]) - 1):
                assert self.featuresPossibleValues[f][v+1] >= self.featuresPossibleValues[f][v] + eps
    ...
    def initSolution(self):
        ...
        for f in range(self.nFeatures):
            if self.featuresType[f] == FeatureType.Binary:
                self.x_var_sol[f] = self.model.addVar(vtype=GRB.BINARY,name="x_f"+str(f))
            elif self.featuresType[f] == FeatureType.Numeric:
                self.x_var_sol[f] = self.model.addVar(lb=0.0,ub=1.0,vtype=GRB.CONTINUOUS,name="x_f"+str(f))
                ...
                linearCombination = gp.LinExpr(self.featuresPossibleValues[f][0])
                for l in range(1,len(self.featuresPossibleValues[f])):
                    self.discreteFeaturesLevel_var[f][l] = self.model.addVar(lb=0, ub=1, vtype=GRB.BINARY,name="level_f"+str(f)+"_l"+str(l))
                    ...
                    if l > 1:
                        self.discreteFeaturesLevelLinearOrderConstr[f][l] = self.model.addConstr(
                            ...
                            "discreteFeaturesLevelLinearOrderConstr_f" + str(f) + "_l" + str(l)
                        )
                    linearCombination += self.discreteFeaturesLevel_var[f][l] * (self.featuresPossibleValues[f][l] - self.featuresPossibleValues[f][l-1])
    ...
    def initObjectiveOfFeature(self, f):
        if self.featuresType[f] == FeatureType.CategoricalNonOneHot:
            self.absoluteValueVar[f] = self.model.addVar(lb=0.0,ub=GRB.INFINITY, vtype=GRB.BINARY,name="modified_CategoricalNonOneHot_f"+str(f))
            self.absoluteValueLeftConstr[f]= self.model.addConstr(
                self.absoluteValueVar[f] * (max(self.featuresPossibleValues[f]) -self.x0[0][f]) >= self.x_var_sol[f] - self.x0[0][f], "absoluteValueLeftConstr_f"+str(f)
            )
            self.absoluteValueRightConstr[f] = self.model.addConstr(
                self.absoluteValueVar[f] * (min(self.featuresPossibleValues[f]) + self.x0[0][f]) >= - self.x_var_sol[f] + self.x0[0][f], "absoluteValueRightConstr_f"+str(f)
            )
            self.obj += self.absoluteValueVar[f]
        elif self.objectiveNorm == 0 or self.objectiveNorm == 1:
            if self.objectiveNorm==0:
                self.absoluteValueVar[f] = self.model.addVar(lb=0.0,ub=GRB.INFINITY, vtype=GRB.BINARY,name="modified_f"+str(f))
            else:
                self.absoluteValueVar[f] = self.model.addVar(lb=0.0,ub=GRB.INFINITY, vtype=GRB.CONTINUOUS,name="abs_value_f"+str(f))
            self.absoluteValueLeftConstr[f]= self.model.addConstr(
                self.absoluteValueVar[f] >= self.x_var_sol[f] - self.x0[0][f], "absoluteValueLeftConstr_f"+str(f)
            )
            self.absoluteValueRightConstr[f] = self.model.addConstr(
                self.absoluteValueVar[f] >= - self.x_var_sol[f] + self.x0[0][f], "absoluteValueRightConstr_f"+str(f)
            )
            self.obj += self.absoluteValueVar[f]
        elif self.objectiveNorm == 2:
            self.obj += (self.x_var_sol[f] - gp.LinExpr(self.x0[0][f])) * (self.x_var_sol[f] - gp.LinExpr(self.x0[0][f]))
        else:
