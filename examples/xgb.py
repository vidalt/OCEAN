...
def main(explainers: dict[str, type[BaseExplainer]]) -> None:
    ...
    clf = fit_model(args, data, target)
    times: dict[str, pd.Series[float]] = {}
    for name, explainer in explainers.items():
        exp = build_explainer(name, explainer, args, clf, mapper)
        queries = generate_queries(args, clf, data)
...
def fit_model(
    ...
    with CONSOLE.status("[bold blue]Fitting a XGBoost model[/bold blue]"):
        clf = XGBClassifier(
            n_estimators=args.n_estimators,
            random_state=args.seed,
            max_depth=args.max_depth,
...
def build_explainer(
    ...
    with CONSOLE.status("[bold blue]Building the Explainer[/bold blue]"):
        ...
        if name == "mip":
            ...
            exp = explainer(clf, mapper=mapper, env=ENV)  # type: ignore  # noqa: PGH003
        else:
            exp = explainer(clf, mapper=mapper)  # type: ignore  # noqa: PGH003
        end = time.time()
